...
\subsection{Defining the Pinned Edge Dislocation (Task 1.6)}
...
The critical shear stress \(\tau_{crit}\) can be estimated using \ref{eq:tau_crit}. With \(\alpha = 0.5\), \(\mu = 25.0~MPa\), \(b = 2.5525~\AA\) and a length of \(L = 80 * 1.2183007689602674~nm \). Resulting in an estimate value of \(\tau_{crit} = 55.0~MPa\).
