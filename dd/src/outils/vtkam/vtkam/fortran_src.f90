...
subroutine readfilmstepdata(mode, bveclin,bvecnor, nsegm, avalue, ldis_act, slipsysact,curvact,  &
...
integer(kind=DPI)                , intent(in)   :: slipsysact
...
elseif (mode == 2) then
  ...
  call reorder_microstructure(nsegm, slipsysact,bveclin,bvecnor,iseg,tabvois,slipsystem, junctt, &
...
subroutine reorder_microstructure(nsegm,slipsysact, bveclin,bvecnor,iseg,tabvois,slipsystem, &
...
if (nsegm < nsegmax) then
  ...
  do dislo = 1, nsegm
    if (slipsysact == int((iseg(5,dislo)-1)/8)+1 .or. slipsysact >12) then
